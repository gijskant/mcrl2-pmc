...
void formularewr(const std::string& input_filename,
              const std::string& output_filename,
              const std::string& lps_filename,
              const std::string& network_filename,
              formularewr_options options = formularewr_options()
             )
{
  // load formula file
  ...
  mCRL2log(log::verbose) << "rewriting quantifiers..." << std::endl;
  state_formulas::state_formula result = state_formulas::rewrite_quantifiers(formula, lps.data());
  mCRL2log(log::debug) << "intermediate result: " << pp(result) << std::endl;
  mCRL2log(log::verbose) << "simplifying formula..." << std::endl;
  result = state_formulas::simplify(result, lps.data());
  if (options.unfold_recursion)
  {
    mCRL2log(log::verbose) << "unfolding unguarded recursion..." << std::endl;
    result = state_formulas::unfold_recursion(result, lps.data());
  }
